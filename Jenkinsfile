        ...
        stage('Deploy to Production'){
            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'prod_credential', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                    script {
                        sh "sshpass -p '$PASSWORD' -v ssh -o StrictHostKeyChecking=no $USERNAME@$PROD_IP \"docker pull willbla/train-schedule:${env.BUILD_NUMBER}\""
                        try {
                        ...
                        sh "sshpass -p '$PASSWORD' -v ssh -o StrictHostKeyChecking=no $USERNAME@$PROD_IP \"docker run --restart always --name train-schedule -p 8080:8080 -d willbla/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
